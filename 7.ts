...
- 왜 타입에러가 발생했을까?
    ...
    swap 함수의 두 parameter 모두 어떤 타입의 데이터(User, Admin, number, string 등...)가 들어올지 모르고, 
    리턴값으로 두 parameter의 위치가 바뀌어 또 서로 타입을 바꿔줘야 하기 때문에 에러가 발생했다. 

- 어떻게 해결할 수 있었을까? 

    타입을 미리 알 수 없는 경우이므로 함수에 parameter가 들어올 때 타입을 추론해줄 수 있는 generic을 사용했다. 
